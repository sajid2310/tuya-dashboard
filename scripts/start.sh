...
# Usage: ./scripts/start.sh   (respects PORT / DATA_DIR / APP_SECRET_KEY env vars if set)
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
  ...
  echo "Run ./scripts/stop.sh first if you want to restart it."
  exit 0
fi
...
if ! "$PYTHON_BIN" -c "import flask, tinytuya, cryptography, psutil" >/dev/null 2>&1; then
  echo "Dependencies look missing. Install them with:"
